...
const CustomerLogin = () => {
    ...
    const handleSetCookie = () => {
        setCookie('Username', username, { path: '/customer' });
        setCookie('Password', encodeCredential(password), { path: '/customer' });
    };
    ...
    const handleSubmit = (event) => {
        axios({
            method: 'post',
            url: 'https://market-0123.herokuapp.com/customer/login',
            data:
            {  
                username: username,
                password: password
                ...
                <div className="login-auth-form">
                    <div className="auth-form__container">
                        ...
                        <div className="auth-form__header">
                            ...
                            <Link to="/customer/register" className="auth-form__switch-btn">Đăng ký</Link>
                        </div>
                        ...
                        <div className="auth-form_controls">
                            <Link to="/customer" className="btn btn--normal auth-form_controls-back">Trở lại</Link>
                            <button type="submit" onClick={localStorage.setItem("customer_id", user.id)} className="btn btn--primary">Đăng nhập</button>
                        </div>

                        <div className="auth-form__aside">
                            <p className="auth-form__register-text">
                                Bạn chưa có tài khoản?{' '}
                                <Link to="/customer/register" className="auth-form__register-link">Đăng ký ngay</Link>
                            </p>
                        </div>
